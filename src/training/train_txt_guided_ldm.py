...
def validate(latent_encoder: nn.Module,
    ...
    with torch.no_grad():
        for step in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast(dtype=dtype):
                result_cfg_15 = torch.cat(list(diffuzz.sample(diffusion_model, {
                ...
                result_cfg_15_uncond = torch.cat(list(diffuzz.sample(diffusion_model, {
                    'encoder_hidden_states': clip_text_embeddings_uncond, 'return_dict': False
                }, latents.shape, unconditional_inputs={
                    'encoder_hidden_states': clip_text_embeddings_uncond, 'return_dict': False
                }, cfg=1.5, t_scaler=(256 // 8) / 256)))

                result_cfg_7 = torch.cat(list(diffuzz.sample(diffusion_model, {
                    'encoder_hidden_states': clip_text_embeddings, 'return_dict': False
                }, latents.shape, unconditional_inputs={
                    'encoder_hidden_states': clip_text_embeddings_uncond, 'return_dict': False

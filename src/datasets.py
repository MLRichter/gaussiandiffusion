...
from src.fast_imagenet import ImageNetDatasetH5
...
def fast_imagenet1k(data_path: str = "../../ilsvrc2012.hdf5", subset: str = "train", size: int = 256, crop_size: int = 256):
    transform = transforms(size, crop_size)
    dataset = ImageNetDatasetH5(data_path, subset, transform=transform)
    subset = dataset.resolution_subset(size)
    return subset
...
def common_canvas(data_path: str = HORRIBLE_CC_FILEPATH, subset: str = "train", size: int = 256, crop_size: int = 256):
    ...
    if subset == "train":
        size = len(dataset)
        subset = torch.utils.data.Subset(dataset=dataset, indices=list(range(size - 10000)))
    elif subset == "val":
        size = len(dataset)
        subset = torch.utils.data.Subset(dataset=dataset, indices=list(range(size - 10000, size)))
    else:
        subset = dataset
    return subset
